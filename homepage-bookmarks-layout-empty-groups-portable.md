# layout 空组合并/清理代码分析（整理版，可移植）

本文档聚焦 servicesResponse 中「只在 settings.layout 声明、没有实际服务」的三类场景，对比其代码执行路径与最终结果，所有结论均附有相对路径和行号依据。

---

## 一、执行顺序核心依据

代码位置：`src/utils/config/api-response.js` L158-L256
...
代码位置：`src/utils/config/service-helpers.js` L710-L725
...
对应测试用例：`src/utils/config/api-response.test.js` L166-L193
...
对应测试用例：`src/utils/config/api-response.test.js` L195-L235
...
代码位置：`src/utils/config/api-response.js` L27-L71（`bookmarksResponse()` 函数）
...
1. 书签响应里根本不会出现该组
2. 前端 `src/pages/index.jsx` L301-L303 的 `layoutGroups` 中，`services.find(name)` 找不到，`bookmarks.find(name)` 也找不到 → 返回 undefined
...
代码位置：`src/utils/config/api-response.js` L123-L134
